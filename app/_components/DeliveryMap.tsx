 ...
 * 정확하다. 다만 Client ID가 있어야 하고 NCP 콘솔에 도메인이 등록돼 있어야 한다.
 * 키가 없거나 인증이 실패하면 OpenStreetMap 임베드로 떨어진다. 팀원 로컬이나
 * 프리뷰 배포에서 키가 없다고 지도가 아예 사라지면 안 된다.
 *
...
function NaverMap({
  ...
  const clientId = process.env.NEXT_PUBLIC_NAVER_MAP_CLIENT_ID as string;
