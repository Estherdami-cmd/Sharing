...
import { btnPrimary, caption, card, field, label, pageDesc, pageTitle, toneBadge } from "../ui";
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-col gap-5">
      <header className="text-center">
        ...
        <p className={pageDesc}>신청할 때 쓴 전화번호로 지금까지 낸 신청을 확인해요</p>
      </header>
      ...
      <div className={card}>
        ...
        {state === "error" && (
          <p className="text-[13px] text-danger-fg">연결에 문제가 있어요. 다시 시도해주세요</p>
        )}
      </div>
      ...
      {results.length > 0 && (
        <div className="flex flex-col gap-3">
          {results.map((app) => (
            <Link key={app.id} href={`/complete/${app.id}`} className={`${card} cursor-pointer hover:border-primary-300`}>
                ...
                <div className="min-w-0">
                  ...
                  {app.confirmedDate ? (
                    ...
                    <p className={`mt-1 ${caption}`}>
                      {app.candidateDates.map((c) => formatKoreanDate(c.date)).join(" · ")} 중 조율
                    </p>
                  )}
                </div>
