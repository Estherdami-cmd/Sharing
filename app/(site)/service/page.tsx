    ...
    <PageShell>
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
        ...
        <div className={card}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
            <img
              ...
              className="h-28 w-full shrink-0 rounded-xl border border-neutral-200 object-cover object-top sm:h-24 sm:w-36"
            />
            <p className={caption}>
        ...
        <div className={card}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
            {/*
              4장을 옆으로 늘어놓으면 카드가 너무 커져서 가로 스크롤로 좁혔다. 세로로
              쌓으면(sm:flex-col) 오른쪽 글 옆에 빈 공간만 길게 남아서, 폭을 좁게 고정하고
              가로 줄은 그대로 유지한다.
            */}
            <div className="-mx-5 flex shrink-0 gap-2 overflow-x-auto px-5 pb-1 sm:mx-0 sm:w-48 sm:px-0">
              {STEPS.map((s) => (
                <div key={s.step} className="flex w-20 shrink-0 flex-col gap-1">
                  <img
                    src={s.src}
                    alt={`${s.step} 화면 — ${s.desc}`}
                    className="h-14 w-full rounded-lg border border-neutral-200 object-cover object-top"
                  />
                  <p className="text-[10px] font-bold text-neutral-700">{s.step}</p>
                </div>
              ))}
            </div>
            <p className={caption}>
